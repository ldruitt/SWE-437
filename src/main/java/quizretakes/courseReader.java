...
public class courseReader {
	public static CourseBean read(String filename) throws IOException,
		...
		for(int i = 0; i < nodeList.getLength(); i++) {
			...
			if(node.getNodeType() == Node.ELEMENT_NODE) {
				...
				String courseID = elem.getElementsByTagName("courseID").item(0).getChildNodes()
						.item(0).getNodeValue();
				System.out.println("course Reader, courseID: " + courseID);
				String courseTitle = elem.getElementsByTagName("courseTitle").item(0)
						.getChildNodes().item(0).getNodeValue();
				String retakeDuration = elem.getElementsByTagName("retakeDuration").item(0)
						.getChildNodes().item(0).getNodeValue();
				String dataLocation = elem.getElementsByTagName("dataLocation").item(0)
						.getChildNodes().item(0).getNodeValue();

				// startSkipMonth is an integer 1..12
				int startSkipMonth = Integer.parseInt(elem.getElementsByTagName("startSkipMonth")
						.item(0).getChildNodes().item(0).getNodeValue());
				// startSkipDay is integer 1..31
				int startSkipDay = Integer.parseInt(elem.getElementsByTagName("startSkipDay").item
						(0).getChildNodes().item(0).getNodeValue());

				// endSkipMonth is an integer 1..12
				int endSkipMonth = Integer.parseInt(elem.getElementsByTagName("endSkipMonth").item
						(0).getChildNodes().item(0).getNodeValue());
				// endSkipDay is integer 1..31
				int endSkipDay = Integer.parseInt(elem.getElementsByTagName("endSkipDay").item(0)
						.getChildNodes().item(0).getNodeValue());
