...
public class retakesReader {
	public static retakes read(String filename) throws IOException, ParserConfigurationException,
			SAXException {
		...
		for(int i = 0; i < nodeList.getLength(); i++) {
			...
			if(node.getNodeType() == Node.ELEMENT_NODE) {
				...
				String location = elem.getElementsByTagName("location").item(0).getChildNodes()
						.item(0).getNodeValue();
